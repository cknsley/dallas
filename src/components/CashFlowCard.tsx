...
export default function CashFlowCard({ state, range }: { state: AppState; range: Range }) {
  ...
  return (
    <section className={`cashflow${positive ? "" : " negative"}`}>
      <div className="cf-main">
        <div className="cf-label">Trésorerie · {range.label}</div>
        <div className="cf-net num">{positive ? "+" : "−"}{eur(Math.abs(cf.net))}</div>
        <div className="cf-sub">
          {positive ? "Entré de plus que sorti sur la période" : "Sorti de plus que rentré sur la période"}
          {cf.pending > 0 && (
